  ...
  if (isLoading) {
    return (
      <div className={`flex items-center ${className}`}>
        {/* Skeleton on white pill — dark gray pulse, not white/20 */}
        <div className="h-3 w-12 animate-pulse rounded-full bg-black/15" />
      </div>
    );
  }
    ...
    <div className={`flex items-center ${className}`}>
      ...
      >
        ...
        <motion.span
          ...
          transition={shouldReduceMotion ? { duration: 0 } : { duration: 0.3, ease: EASE_OUT_CUBIC }}
        >
          {displayCount.toLocaleString()}
        </motion.span>
        <span className="text-sm font-normal" style={{ color: "oklch(55% 0.005 260)" }}>
          stars
        </span>
      </motion.div>
    </div>
