...
describe("long-shot visibility helpers", () => {
  it("counts long-shot rows hidden by the client-side toggle", () => {
    const programs = [row("doesnt-matter"), row("delulu"), row("doesnt-matter")];
  ...
  it("detects predictions where every returned row is hidden as a long-shot", () => {
    expect(
      hasOnlyClientHiddenLongShots([row("doesnt-matter"), row("doesnt-matter")], false),
    ).toBe(true);
    expect(
      hasOnlyClientHiddenLongShots([row("doesnt-matter"), row("delulu")], false),
    ).toBe(false);
    expect(hasOnlyClientHiddenLongShots([row("doesnt-matter")], true)).toBe(false);
  });
